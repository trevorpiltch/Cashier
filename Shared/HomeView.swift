...
struct HomeView: View {
            ...
            .navigationBarItems(leading:
                ...
                .sheet(isPresented: $showSettings) {
                    SettingsView()
                                ...
                                trailing:
                                    Button(action: {
                                        itemModel.date = Date(timeInterval: -10000000, since: Date())
                                        itemModel.amount = 20
                                        itemModel.item = "Chicken"
    ...
    func updateGraph() {
        ...
        for i in itemModel.timeSortedData.indices {
            let currentDateFormatted = dateFormatter.string(from: itemModel.getValue(obj: itemModel.timeSortedData[i]).date)
            
            amounts.append((currentDateFormatted, (itemModel.getValue(obj: itemModel.timeSortedData[i]).amount)))
...
struct DashboardView_Previews: PreviewProvider {
    static var previews: some View {
        HomeView(itemModel: ItemModel(), cardModel: CardModel(company: "Norway", number: "0000-0000", gradient: 0, name: "", id: UUID()))
    }
}
